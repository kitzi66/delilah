...
function validaToken(req, res, next) {
    try {
        if (req.headers.authorization == undefined && req.method == 'POST') {
            req.usuario = { role: 0 }
            return next()
        } else {
...
router.put('/:id', async function (req, res) {
    try {
        ...
        if(req.params.id != req.usuario.id && req.usuario.role != 1){
            throw 'Usted no puede actualizar este usuario';
        }
        ...
        const [rows, fields] = await req.db.execute('UPDATE usuarios SET username=?, nombre=?, email=?, telefono=?, direccion=?, password=?, role=? WHERE id=?',
            [username, nombre, email, telefono, direccion, password, newRole, req.params.id])
